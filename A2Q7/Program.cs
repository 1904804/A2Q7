...
namespace A2Q7
{
    class Program
    {
        ...
        {
            int[] values = new int[10]; //setup the array with max of 10 entries
            ...
            if (entries%2 ==0)//checks if entries is odd or even to perform the correct calculation
            {
                ...
                int midNum = arraySize / 2 + entries; // to overcome a bug where a number of zero`s are added to to sorted array
